 ...
 * - Subcategories (CategoryHero subnav, CategorySidebar "Browse" list) now
 *   come from real WP child categories via get_categories( 'parent' => ... )
 *   instead of the hardcoded SUBCATEGORIES JS array, so they stay in sync
 *   with whatever hierarchy you set up in wp-admin. Term descriptions are
 *   pulled from category_description() rather than a hardcoded JS map.
...
$sassystrides_subcategories = ( $sassystrides_parent_term instanceof WP_Term )
	? get_categories(
		array(
			'parent'     => $sassystrides_parent_term->term_id,
			'hide_empty' => false,
		)
	)
	: array();
		...
		<section class="category-hero editorial-container border border-ink/10 bg-paper-grain">
			...
			<nav class="category-subnav flex overflow-x-auto border-t border-ink/10 bg-ivory">
				...
				<?php foreach ( $sassystrides_subcategories as $sassystrides_subcategory ) : ?>
					<?php $sassystrides_sub_is_active = ( $sassystrides_subcategory->term_id === $sassystrides_current_term->term_id ); ?>
					<a
						href="<?php echo esc_url( get_category_link( $sassystrides_subcategory ) ); ?>"
						class="category-subnav__link<?php echo $sassystrides_sub_is_active ? ' is-active' : ''; ?>"
						<?php echo $sassystrides_sub_is_active ? ' aria-current="page"' : ''; ?>
					>
				...
				<section class="border border-ink/10 bg-porcelain p-5">
					...
					<nav class="category-sidebar__nav">
						<?php foreach ( $sassystrides_subcategories as $sassystrides_browse_item ) : ?>
							<?php $sassystrides_browse_is_active = ( $sassystrides_browse_item->term_id === $sassystrides_current_term->term_id ); ?>
							<a
								href="<?php echo esc_url( get_category_link( $sassystrides_browse_item ) ); ?>"
